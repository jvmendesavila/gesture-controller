...
export default class Service {
  ...
  async handBlinked(video) {
    ...
    for (const prediction of predictions) {
      ...
      // Detect what eye is closed
      const leftBlink = leftEAR <= EAR_THRESHOLD
      const rightBlink = rightEAR <= EAR_THRESHOLD
      // const blinked = leftBlink && rightBlink
      const onlyRightBlinked = !leftBlink && rightBlink
      const onlyLeftBlinked = leftBlink && !rightBlink
      if (!onlyRightBlinked && !onlyLeftBlinked) continue
